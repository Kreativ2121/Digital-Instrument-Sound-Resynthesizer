...
%PHASE_CALCULATION_ Function calculatic a cubic polynomial for phase
...
% 3.24
eta = (3/pow(S,2))*(phase_now - phase_last - freq_last*S + 2*pi*M)-(1/S)*(freq_now-freq_last);
jota = (-2/pow(S,3))*(phase_now - phase_last - freq_last*S + 2*pi*M)+(1/pow(S,2))*(freq_now-freq_last);
