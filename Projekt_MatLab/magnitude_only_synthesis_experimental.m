...
% 
%     AmpInst = 0.8;
%     f_start = Trajectories(4*(tra-1),1);
%     f_end = Trajectories(4*(tra),1);
%     f_in = linspace(f_start, f_end, hopsize);
%     % phase_in = cumsum(f_in);
%     for i = 1:hopsize
%         AmpSum = 0;
%         AmpSum = AmpSum + AmpInst*cos(2*pi*f_in(i)*stepcounter/fs);
%         % AmpSum = AmpSum + AmpInst*sin(2*pi*(2*stepcounter-cos(stepcounter)));
% 
%         OutputAmp((cutoff*AmpRamp)+stepcounter-99999) = AmpSum;
%         stepcounter = stepcounter + 1;
%         % if(stepcounter == cutoff)
%         %     AmpRamp = AmpRamp+1;
%         %     stepcounter = 1;
%         % end
%     end
